document.addEventListener('DOMContentLoaded', () => {
  ...
  if (originSection && !prefersReduced) {
    const isMobileOrigin = window.matchMedia('(max-width: 640px)').matches;

    if (isMobileOrigin) {
      // Gepinntes Scrollen ("pin") ist auf Mobilgeräten — vor allem iOS Safari
      // mit seiner sich ständig verändernden Adressleiste/Viewport-Höhe —
      // unzuverlässig: der Effekt blieb "hängen" und die Absätze zeigten nur
      // ihren gedimmten Zwischenzustand statt sauber nacheinander zu erscheinen.
      // Deshalb hier bewusst kein Pin, sondern ein einfacher, robuster Reveal
      // wie im Rest der Seite.
      gsap.fromTo('#originMoon', { opacity: 0, scale: 0.4 }, {
        opacity: 1, scale: 1, duration: 1, ease: 'power3.out',
        scrollTrigger: { trigger: '#originMoon', start: 'top 90%', toggleActions: 'play none none reverse' }
      });
      ['#originBadge', '#originEyebrow', '#originHeading', '#originP1', '#originP2', '#originP3', '#originSignoff'].forEach((sel) => {
        gsap.fromTo(sel, { opacity: 0, y: 30 }, {
          opacity: 1, y: 0, duration: 0.8, ease: 'power3.out',
          scrollTrigger: { trigger: sel, start: 'top 90%', toggleActions: 'play none none reverse' }
        });
      });
    } else {
